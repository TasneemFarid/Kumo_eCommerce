								...
								<div class="killore--block-link-content">
									<ul>
                                        @foreach ($categories as $category)
    										<li><a href="{{route('category_product', $category->id)}}"><img width="25" height="25" src="{{asset('uploads/category')}}/{{$category->category_image}}" alt="">  &nbsp;{{$category->category_name}}</a></li>
                                        @endforeach
									</ul>
								</div>
							</div>
                                ...
                                <div class="product_grid card b-0">
                                    ...
                                    <div class="card-body p-0">
                                        <div class="shop_thumb position-relative">
                                            <a class="card-img-top d-block overflow-hidden" href="{{route('product_details', $product->slug)}}"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$product->preview}}" alt="..."></a>
                                        </div>
                                    </div>
                                    <div class="card-footer b-0 p-0 pt-2 bg-white d-flex align-items-start justify-content-between">
                                        <div class="text-left">
                                            <div class="text-left">
                                                <div class="elso_titl"><span class="small">{{$product->rel_to_cat->category_name}}</span></div>
                                                <h5 class="fs-md mb-0 lh-1 mb-1"><a href="{{route('product_details', $product->slug)}}">{{$product->product_name}}</a></h5>
                                                ...
                                                <div class="elis_rty">
                                                    @if ($product->discount)
                                                        <span class="ft-medium text-muted line-through fs-md mr-2">TK {{$product->price}}</span>
                                                    @endif
                        ...
                        @foreach ($categories as $category)
                            <div class="col-xl-2 col-lg-2 col-md-3 col-sm-6 col-4">
                                <div class="cats_side_wrap text-center mx-auto mb-3">
                                    <div class="sl_cat_01"><div class="d-inline-flex align-items-center justify-content-center p-4 circle mb-2 border"><a href="{{route('category_product', $category->id)}}" class="d-block"><img src="{{asset('uploads/category')}}/{{$category->category_image}}" class="img-fluid" width="40" alt=""></a></div></div>
                                    <div class="sl_cat_02"><h6 class="m-0 ft-medium fs-sm"><a href="{{route('category_product', $category->id)}}">{{$category->category_name}}</a></h6></div>
                                </div>
                            </div>
                        @endforeach
					</div>
									...
									<div class="single_review">
										...
										<div class="sng_rev_caption text-center">
											<div class="rev_desc mb-4">
												<p class="fs-md">{{$review->review}}</p>
											</div>
											<div class="rev_author">
												<h4 class="mb-0">--<i>{{$review->rel_to_customer->name}}</i></h4>
												<span class="fs-sm">{{$review->rel_to_customer->country}}</span>
											</div>
										</div>
									</div>
								@endforeach
				...
				<div class="container">
					...
					<div class="row">
							...
							<div class="ftr-content">
								...
								<div class="product_grid row">
									<div class="col-xl-4 col-lg-5 col-md-5 col-4">
										<div class="shop_thumb position-relative">
											<a class="card-img-top d-block overflow-hidden" href="{{route('product_details', $top_selling->rel_to_product->slug)}}"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$top_selling->rel_to_product->preview}}" alt="..."></a>
										</div>
									</div>
									<div class="col-xl-8 col-lg-7 col-md-7 col-8 pl-0">
										<div class="text-left mfliud">
											<div class="elso_titl"><span class="small">{{$top_selling->rel_to_product->rel_to_cat->category_name}}</span></div>
											<h5 class="fs-md mb-0 lh-1 mb-1 ft-medium"><a href="{{route('product_details', $top_selling->rel_to_product->slug)}}">{{$top_selling->rel_to_product->product_name}}</a></h5>
											<div class="star-rating align-items-center d-flex justify-content-left mb-2 p-0">
												@php
												$total_review = App\Models\OrderProduct::where('product_id', $top_selling->product_id)->where('review', '!=', null)->count();
												$total_star = App\Models\OrderProduct::where('product_id', $top_selling->product_id)->where('review', '!=', null)->sum('star');
												if($total_review == 0){
													$ratings = 0;
												}else{
											...
											<div class="elis_rty"><span class="ft-bold text-dark fs-sm">TK {{$top_selling->rel_to_product->after_discount}}</span></div>
										</div>
									</div>
								</div>
							...
							<div class="ftr-content">
                                ...
                                @foreach ($feat_products as $feat_product)
                                    <div class="product_grid row">
                                        <div class="col-xl-4 col-lg-5 col-md-5 col-4">
                                            <div class="shop_thumb position-relative">
                                                <a class="card-img-top d-block overflow-hidden" href="shop-single-v1.html"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$feat_product->preview}}" alt="..."></a>
                                            </div>
                                        </div>
                                        <div class="col-xl-8 col-lg-7 col-md-7 col-8 pl-0">
                                            <div class="text-left mfliud">
                                                <div class="elso_titl"><span class="small">{{$feat_product->rel_to_cat->category_name}}</span></div>
                                                ...
                                                <div class="elis_rty"><span class="ft-bold text-dark fs-sm">TK {{$feat_product->after_discount}}</span></div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
						...
						<div class="col-xl-4 col-lg-4 col-md-4 col-sm-12">
							...
							<div class="ftr-content">
								<!-- Single Item -->
								@foreach ($recently_viewed_products->take(3) as $recently_viewed)
								<div class="product_grid row">
									<div class="col-xl-4 col-lg-5 col-md-5 col-4">
										<div class="shop_thumb position-relative">
											<a class="card-img-top d-block overflow-hidden" href="shop-single-v1.html"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$recently_viewed->preview}}" alt="..."></a>
										</div>
									</div>
									<div class="col-xl-8 col-lg-7 col-md-7 col-8 pl-0">
										<div class="text-left mfliud">
											<div class="elso_titl"><span class="small">{{$recently_viewed->rel_to_cat->category_name}}</span></div>
											<h5 class="fs-md mb-0 lh-1 mb-1 ft-medium"><a href="shop-single-v1.html">{{$recently_viewed->product_name}}</a></h5>
											<div class="star-rating align-items-center d-flex justify-content-left mb-2 p-0">
												@php
												$total_review = App\Models\OrderProduct::where('product_id', $recently_viewed->id)->where('review', '!=', null)->count();
												$total_star = App\Models\OrderProduct::where('product_id', $recently_viewed->id)->where('review', '!=', null)->sum('star');
												if($total_review == 0){
													$ratings = 0;
												}else{
													$ratings = round($total_star/$total_review);
											...
											<div class="elis_rty"><span class="ft-bold text-dark fs-sm">TK {{$recently_viewed->after_discount}}</span></div>
										</div>
									</div>
								</div>
								@endforeach
