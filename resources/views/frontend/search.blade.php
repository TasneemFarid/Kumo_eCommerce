                                        ...
                                        <div class="card-body pt-0">
                                            <div class="inner_widget_link">
                                                <ul class="no-ul-list">
                                                    ...
                                                    <li>
                                                        <input id="brands{{$brand->id}}" class="brand" name="brands" type="radio" value="{{$brand->id}}" {{$brand->id == @$_GET['brand']?'checked':''}}>
                                                        <label for="brands{{$brand->id}}" class="checkbox-custom-label">{{$brand->brand_name}}<span>{{App\Models\Product::where('brand', $brand->id)->count()}}</span></span></label>
                                                    </li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                            ...
                                            <div class="text-left">
                                                @foreach ($colors as $color)
                                                    <div class="form-check form-option form-check-inline mb-1">
                                                        <input class="color" type="radio" name="color_id" id="white{{$color->id}}" value="{{$color->id}}" {{$color->id == @$_GET['color']?'checked':''}}>
                                                        <label class="form-option-label rounded-circle" for="white{{$color->id}}"><span class="form-option-color rounded-circle" style="background: {{$color->color_code}}"></span></label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                            ...
                                            <div class="text-left pb-0 pt-2">
                                                @foreach ($sizes as $size)
                                                    <div class="form-check form-option form-check-inline mb-2">
                                                        <input class="size" type="radio" name="size_id" id="{{$size->id}}s" value="{{$size->id}}" {{$size->id == @$_GET['size']?'checked':''}}>
                                                        <label class="form-option-label" for="{{$size->id}}s">{{$size->size_name}}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                        ...
                        <div class="product_grid card b-0">
                            ...
                            <div class="card-body p-0">
                                <div class="shop_thumb position-relative">
                                    <a class="card-img-top d-block overflow-hidden" href="{{route('product_details', $search_product->slug)}}"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$search_product->preview}}" alt="..."></a>
                                </div>
                            </div>
                            <div class="card-footer b-0 p-0 pt-2 bg-white">
                                
                                <div class="text-left">
                                    <h5 class="fw-bolder fs-md mb-0 lh-1 mb-1"><a href="shop-single-v1.html">{{$search_product->product_name}}</a></h5>
                                    <div class="elis_rty"><span class="ft-bold text-dark fs-sm">TK {{$search_product->after_discount}}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('footer_script')
    {{-- <script>
        $('#search_btn').click(function(){
            ...
            var link = "{{route('search')}}" + "?q=" + search_input + "&min=" + min + "&max=" + max + "&category_id=" + category_id + "&color_id=" + color_id + "&size_id=" + size_id + "&sort=" + sort + "&brand=" + brand;
