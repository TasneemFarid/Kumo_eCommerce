            ...
            <div class="col-xl-7 col-lg-6 col-md-12 col-sm-12">
                <div class="prd_details pl-3">
                    <form action="{{route('cart_store')}}" method="POST">
                        ...
                        <div class="prt_01 mb-1"><span class="text-light bg-info rounded px-2 py-1">{{App\Models\Category::withTrashed()->where('id', $product_info->first()->category_id)->withTrashed()->first()->category_name}}</span></div>
                        <div class="prt_02 mb-3">
                            ...
                            <div class="text-left">
                                <div class="star-rating align-items-center d-flex justify-content-left mb-1 p-0">
                                    ...
                                    @endfor
                                    <span class="small">{{$ratings}}</span>
                                </div>
                                <div class="elis_rty"><span class="ft-medium text-muted line-through fs-md mr-2">TK {{$product_info->first()->price}}</span><span class="ft-bold theme-cl fs-lg mr-2">TK {{$product_info->first()->after_discount}}</span></div>
                            </div>
                        </div>
                                ...
                                @foreach ($available_colors as $colors)
                                    @if ($colors->rel_to_color->color_name == 'NA')
                                        <div class="form-check form-option form-check-inline mb-1">
                                            ...
                                            <label style="background: {{$colors->rel_to_color->color_code}}" class="form-option-label rounded-circle" for="white{{$colors->rel_to_color->id}}"><span class="form-option-color rounded-circle">NA</span></label>
                                        </div>
                                    @else
                                        <div class="form-check form-option form-check-inline mb-1">
                                            <input class="form-check-input color_id" value="{{$colors->rel_to_color->id}}" type="radio" name="color_id" id="white{{$colors->rel_to_color->id}}">
                                            <label style="background: {{$colors->rel_to_color->color_code}}" class="form-option-label rounded-circle" for="white{{$colors->rel_to_color->id}}"><span class="form-option-color rounded-circle"></span></label>
                                        </div>
                                    @endif
                                @endforeach
                                <br>
                        ...
                        <div class="prt_05 mb-4">
                            <div class="form-row mb-7">
                                ...
                                <div class="col-12 col-lg">
                                    <!-- Submit -->
                                    <button type="submit" name="btns" value="1" class="btn btn-block custom-height bg-dark mb-2">
                                ...
                                <div class="col-12 col-lg-auto">
                                    <!-- Wishlist -->
                                    <button type="submit" name="btns" value="2" class="btn custom-height btn-default btn-block mb-2 text-dark">
                    ...
                    <div class="tab-pane fade" id="reviews" role="tabpanel" aria-labelledby="reviews-tab">
                                ...
                                <div class="single_rev d-flex align-items-start br-bottom py-3">
                                    <div class="single_rev_thumb">
                                        @if ($review->rel_to_customer->photo == null)
                                            <img class="img-fluid circle" width="90" src="{{ Avatar::create($review->rel_to_customer->name)->toBase64() }}" />
                                        @else
                                            <img src="{{asset('uploads/customer')}}/{{$review->rel_to_customer->photo}}" class="img-fluid circle" width="90" alt=""/>
                                        @endif
                                    </div>
                                    <div class="single_rev_caption d-flex align-items-start pl-3">
                                        ...
                                        &nbsp;
                                        <div class="my-2">
                                            <img src="{{asset('uploads/review')}}/{{$review->image}}" width="90" alt="">
                                        </div>
                        ...
                        @auth('customerlogin')
                            @if (App\Models\OrderProduct::where('customer_id', Auth::guard('customerlogin')->id())->where('product_id', $product_info->first()->id)->exists())
                                @if (App\Models\OrderProduct::where('customer_id', Auth::guard('customerlogin')->id())->where('product_id', $product_info->first()->id)->whereNotNull('review')->first())
                                <div class="alert alert-warning">
                                    <h3>You have already reviewed.</h3>    
                                </div>
                                @else
                                <div class="reviews_rate">
                                    <form class="row" action="{{route('review_store')}}" method="POST" enctype="multipart/form-data">
                                        @csrf
                            ...
                            @else
                            <div class="alert alert-warning">
                                <h3>You haven't purchased this product yet.</h3>    
                            </div> 
                            @endif
                        @else
                            <div class="alert alert-warning">
                                <h3>Please login to give a review <a href="{{route('customer_login_register')}}" class="float-right btn btn-success">Login</a></h3>    
                            </div>    
                        @endauth
                            ...
                            <div class="product_grid card b-0 mb-0">
                                ...
                                <div class="card-body p-0">
                                    <div class="shop_thumb position-relative">
                                        <a class="card-img-top d-block overflow-hidden" href="{{route('product_details', $related_product->slug)}}"><img class="card-img-top" src="{{asset('uploads/product/preview')}}/{{$related_product->preview}}" alt="..."></a>
                                    </div>
                                </div>
                                <div class="card-footer b-0 p-3 pb-0 d-flex align-items-start justify-content-center">
                                    <div class="text-left">
                                        <div class="text-center">
                                            <h5 class="fw-bolder fs-md mb-0 lh-1 mb-1"><a href="{{route('product_details', $related_product->slug)}}">{{$related_product->product_name}}</a></h5>
                                            <div class="elis_rty"><span class="ft-bold fs-md text-dark">TK {{$related_product->after_discount}}</span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
